...
void TriangularAnisotropicFEMForceField<DataTypes>::reinit()
{
    ...
    // msg_error() << "cache properties";
    for (unsigned int i = 0; i < poissonArray.size(); i++)
    {
        unsigned int y_index = i < youngArray.size()? i:0;
        unsigned int y2_index = i < young2Array.size()? i:0;
        poiss2.push_back( poissonArray[i]*(young2Array[y2_index]/youngArray[y_index]));
    }
    // msg_error() << "preupdate poiss2";
...
/*template<class DataTypes>
void TriangularAnisotropicFEMForceField<DataTypes>::accumulateForceLarge(VecCoord &f, const VecCoord &p, Index elementIndex )
{
    
    Inherited::accumulateForceLarge(f,p,elementIndex);
}*/
